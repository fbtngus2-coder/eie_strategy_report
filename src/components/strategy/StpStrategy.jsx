...
import { Users, Crosshair, Flag } from 'lucide-react';

const StpStrategy = ({ studentInfo, parentsType, targetAudience }) => {
    if (!studentInfo) return null;

    // Logic: Prioritize User Selected Target, otherwise calculate based on students
    const getPrimarySegment = () => {
        if (targetAudience) return { name: targetAudience, count: 0, isSelected: true };

        // V2 Data Structure Support (Flat numbers)
        const { kinder = 0, elem_low = 0, elem_high = 0, middle = 0, high = 0 } = studentInfo || {};

        // Calculate totals directly from numbers
        const eTotal = (elem_low || 0) + (elem_high || 0);
        // If 'middle' is an object (Legacy V1), verify. But for V2 it's a number. 
        // We treat it as number safe logic.
        const mTotal = typeof middle === 'object' ? 0 : (middle || 0); // safe fallback
        ...
        <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl shadow-gray-200/40 mb-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-6 bg-pink-50 rounded-2xl border border-pink-100 flex flex-col h-full hover:shadow-lg transition-shadow">
                    ...
                    <h3 className="font-bold text-pink-900 text-lg mb-3">Positioning (이미지)</h3>
                    <div className="text-sm text-pink-900/80 leading-relaxed flex-1">
                        ...
                        <p className="text-justify">
                            단순히 '영어를 가르치는 곳'을 넘어, 우리 아이의 영어 자신감을 완성하고
                            미래를 함께 설계하는 <strong>'대체 불가능한 교육 파트너'</strong>로서의 브랜드 이미지를 구축하십시오.
                        </p>
                    </div>
                </div>
            </div>
